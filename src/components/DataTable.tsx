      ...
      >
        ...
        <div className="space-y-2 font-mono-data text-[13px]">
          ...
          <div className="space-y-1 pt-1.5" style={{ borderTop: '1px solid #22252c' }}>
            <div style={{ color: '#888' }}><span style={{ color: '#555' }}>2.</span> 逐发推演（前3发）</div>
...
function TTKShotBreakdown({ data, penRate }: { data: DamageResult; penRate: number }) {
  const shots: { shot: number; preDur: number; mitigation: number; actualDmg: number; cumDmg: number; note: string }[] = [];
  const trueArmorDmg = data.damagePerShotArmor;
  const profile = data.weapon.profiles[data.bulletType];
  const baseDamage = profile?.baseDamage ?? 0;
  ...
  while (hp > 0 && shotNum < 6) {
    ...
    hp -= actualDmg;
    cumDmg += actualDmg;
    shots.push({ shot: shotNum, preDur, mitigation: isArmorRelevant && preDur > 0 ? Math.min(preDur / trueArmorDmg, 1.0) : 0, actualDmg, cumDmg, note });
  }
  ...
  return (
    <div className="pl-3 space-y-1">
      ...
      {shots.map((s) => (
        <div key={s.shot} className="grid grid-cols-5 gap-1 text-[13px]" style={{ color: s.note === '碎甲' ? '#e0a33e' : '#ccc' }}>
      ...
      {data.totalShots > 6 && <div className="text-[11px] pl-3" style={{ color: '#555' }}>... 共 {data.totalShots} 发直至击杀</div>}
    </div>
  );
}
